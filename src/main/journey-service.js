...
function getProfile(db = getDatabase()) {
  return db.prepare('SELECT preferred_name, date_of_birth, updated_at FROM traveler_profile WHERE id = 1').get()
    || { preferred_name: 'Friend', date_of_birth: null, updated_at: null };
}
...
function validateProfile(profile, today = formatLocalDate()) {
  ...
  if (preferredName.length > 80) {
    return { ok: false, error: 'Preferred name must be 80 characters or fewer.' };
  }
  if (dateOfBirth) {
...
function getConsistencyStats(db = getDatabase(), today = formatLocalDate()) {
  ...
  return {
    ...
    openedThisMonthDays: db.prepare('SELECT COUNT(*) AS count FROM app_open_logs WHERE open_date LIKE ?').get(monthPrefix).count,
    openedToday: Boolean(db.prepare('SELECT 1 FROM app_open_logs WHERE open_date = ?').get(today))
  };
}

function getDashboardData(db = getDatabase()) {
  const today = formatLocalDate();
  ensureTrackedDay(today, db);

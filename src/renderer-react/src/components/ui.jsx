...
export function PageHeader({ eyebrow, title, subtitle, actions }) {
  return (
    <header className="mb-7 flex flex-wrap items-start justify-between gap-6">
      <div>
        {eyebrow ? <p className="premium-kicker mb-2 text-xs font-bold uppercase tracking-[0.24em]">{eyebrow}</p> : null}
        <h1 className="premium-page-title text-3xl font-extrabold tracking-normal">{title}</h1>
  ...
  return (
    <div className="rounded-card border border-border bg-raised p-5">
      <div className="mb-4 flex items-center justify-between">
        <p className="premium-kicker text-xs font-bold uppercase tracking-[0.18em]">{label}</p>
...
export function EmptyState({ icon: Icon, title, description, action }) {
  return (
    <div className="flex min-h-40 flex-col items-center justify-center rounded-card border border-dashed border-border bg-editor/50 p-8 text-center">
      ...
      <h3 className="premium-section-title text-sm font-bold">{title}</h3>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(520px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 rounded-panel border border-border bg-panel p-6 shadow-soft focus:outline-none">
          <div className="mb-5 flex items-center justify-between gap-4">
            <Dialog.Title className="premium-section-title text-lg font-extrabold">{title}</Dialog.Title>
...
export function Field({ label, children }) {
  return (
    <label className="block">
      <span className="premium-kicker mb-2 block text-xs font-bold uppercase tracking-[0.16em]">{label}</span>
      {children}
    </label>
  );
}
